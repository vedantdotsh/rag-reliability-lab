> Archived web workflow. This describes the previous dashboard/report interface.
> Use README.md for the current terminal-only basic version.

# Reliability Lab

The repository also includes a local prompt-only general LLM diagnostic. See the
[general evaluation guide](docs/GENERAL_LLM_EVALUATION.md) for frozen repeated runs,
equivalent-prompt robustness, supplied-evidence abstention, and grader controls.
